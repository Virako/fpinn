...
import pygtk
pygtk.require('2.0')
import gtk, gtk.glade, pango, time, mx, mx.DateTime
...
class Fincas(Ventana, VentanaGenerica):
    CLASE = pclases.Finca
    VENTANA = os.path.join("ui", "fincas.glade")
    ...
    def cambiar_plano(self, boton):
        nomfich = utils.dialogo_abrir(titulo = "BUSCAR PLANO FINCA", 
    ...
    def inicializar_ventana(self):
        ...
        pfd = pango.FontDescription("serif,monospace bold condensed 24")
        self.wids['e_nombre'].modify_font(pfd)
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana = "fincas.py")
    ...
    def rellenar_widgets(self):
        ...
        self.objeto.mostrar_imagen_en(self.wids['plano'], MAX = 480)
    ...
    def buscar(self, widget):
        ...
        a_buscar = utils.dialogo_entrada(
            ...
            texto = "Introduzca nombre, provincia, población, polígono"
                    " o parcela:", 
            padre = self.wids['ventana']) 
        if a_buscar != None:
            try:
            ...
            campos_busqueda = (self.clase.q.nombre, 
                               self.clase.q.provincia, 
                               self.clase.q.poblacion, 
                               self.clase.q.poligono, 
                               self.clase.q.parcela)
            subsubcriterios = []
    ...
    def borrar(self, widget):
        ...
        """
        if not utils.dialogo('Esta es una operación peligrosa, ¿está completamente seguro de eliminar %s?' % self.clase.sqlmeta.table.lower(), 
                             'BORRAR', 
                             padre = self.wids['ventana']):
            return
        self.objeto.notificador.desactivar()
        try:
            ...
            self.logger.error("fincas.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
