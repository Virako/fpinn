...
class Familias(Ventana, VentanaGenerica):
    CLASE = pclases.Familia
    VENTANA = os.path.join("ui", "familias.glade")
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana = "familias.py")
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        filas_res = []
        for r in resultados:
            filas_res.append((r.id, r.nombre, r.observaciones))
    ...
    def buscar(self, widget):
        ...
        if a_buscar != None:
            try:
                ...
                ida_buscar = -1
            campos_busqueda = (self.clase.q.nombre, self.clase.q.observaciones)
            subsubcriterios = []
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("familias.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
