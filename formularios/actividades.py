...
###################################################################
## actividades.py - Actividades de jornadas de campo.
###################################################################
##  
###################################################################
## Changelog:
## 7 de enero de 2007 -> Inicio
## 
###################################################################
...
class Actividad(Ventana, VentanaGenerica):
    CLASE = pclases.Actividad
    VENTANA = os.path.join("ui", "actividades.glade")
    def __init__(self, objeto = None, usuario = None):
        ...
        connections = {'b_salir/clicked': self.salir,
                       ...
                       'manipulacion/toggled': 
                            self.verificar_no_campo_y_manipulacion, 
                       'campo/toggled': self.verificar_no_campo_y_manipulacion, 
                      }
    ...
    def verificar_no_campo_y_manipulacion(self, ch):
        """
        Verifica que los checkboxes campo y manipulación no estén a la vez 
        marcados.
        Si es así, anula el que no se ha recibido (que es el último que se 
        ha cambiado, por eso lo recibe este callback, you stupid moron!).
        """
        if (self.wids['campo'].get_active() 
            and self.wids['manipulacion'].get_active()):
            if ch is self.wids['campo']:
                self.wids['manipulacion'].set_active(False)
            elif ch is self.wids['manipulacion']:
                self.wids['campo'].set_active(False)
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana = "actividades.py")
    ...
    def nuevo(self, widget):
        ...
        """
        while 1:
            codigo = utils.dialogo_entrada(titulo = "CÓDIGO ACTIVIDAD", 
            ...
            try:
                ...
                utils.dialogo_info(titulo = "ERROR", 
                                   texto = "El código debe ser numérico.", 
                                   padre = self.wids['ventana'])
            else:
                break
        if self.clase.select(self.clase.q.codigo == codigo).count() > 0:
            utils.dialogo_info(titulo = "ERROR", 
                               texto = "El código %d ya existe." % codigo, 
                               padre = self.wids['ventana'])
            return
        objeto_anterior = self.objeto
        if objeto_anterior != None:
            objeto_anterior.notificador.desactivar()
        self.objeto = self.clase(codigo = codigo) 
    ...
    def buscar(self, widget):
        ...
        if a_buscar != None:
            try:
                ...
                ida_buscar = -1
            campos_busqueda = (self.clase.q.descripcion, )
            subsubcriterios = []
    ...
    def guardar(self, widget):
        ...
        self.objeto.notificador.desactivar()
        # Como no se guardan «campo» y «manipulacion» a la vez, anulo los 
        # dos antes de que se guarde el primero de ellos, no sea que por un 
        # instante ambos se queden a True y salte la excepción del CHECK de 
        # la tabla.
        self.objeto.campo = self.objeto.manipulacion = False
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("actividades.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
