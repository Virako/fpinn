...
###################################################################
## clientes.py - Ventana de clientes.
###################################################################
## NOTAS:
...
class Clientes(Ventana, VentanaGenerica):
    CLASE = pclases.Cliente
    VENTANA = os.path.join("..", "ui", "clientes.glade")
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        id = utils.dialogo_resultado(filas_res,
                                     ...
                                     cabeceras = ('ID', 'Nombre', 'C.I.F./N.I.F.'), 
    ...
    def nuevo(self, widget):
        ...
        self.objeto = self.clase(tarifa = pclases.Tarifa.get_por_defecto()) 
        ...
        utils.dialogo_info('NUEVO CLIENTE CREADO', 
    ...
    def buscar(self, widget):
        ...
        if a_buscar != None:
            try:
                ...
                ida_buscar = -1
            campos_busqueda = (self.clase.q.nombre, self.clase.q.cif)
            subsubcriterios = []
    ...
    def guardar(self, widget):
        ...
        if (not self.objeto.direccionfacturacion and 
            not self.objeto.paisfacturacion and
            not self.objeto.ciudadfacturacion and 
            not self.objeto.provinciafacturacion and 
            not self.objeto.cpfacturacion and 
            not self.objeto.nombref):
            self.objeto.nombref = self.objeto.nombre
            self.objeto.cpfacturacion = self.objeto.cp
            self.objeto.provinciafacturacion = self.objeto.provincia
            self.objeto.ciudadfacturacion = self.objeto.ciudad
            self.objeto.paisfacturacion = self.objeto.pais
            self.objeto.direccionfacturacion = self.objeto.direccion
    ...
    def borrar(self, widget):
        ...
        """
        if not utils.dialogo('¿Eliminar el cliente?', 
                             'BORRAR CLIENTE', 
                             padre = self.wids['ventana']):
            return
        self.objeto.notificador.desactivar()
        try:
            ...
            self.logger.error("cliente.py::borrar -> Cliente ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.id, e))
            utils.dialogo_info(titulo = "CLIENTE NO BORRADO", 
                               texto = "El cliente no se pudo eliminar.\n\nVerifique que no está implicado en facturas o albaranes.",
                               padre = self.wids['ventana'])
            self.actualizar_ventana()
            return
        self.objeto = None
