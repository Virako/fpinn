...
class SeriesFacturasCompra(Ventana, VentanaGenerica):
    CLASE = pclases.SerieFacturasCompra
    VENTANA = os.path.join("ui", "serie_facturas_venta.glade")
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana = "serie_facturas_compra.py")
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        filas_res = []
        for r in resultados:
            filas_res.append((r.id, r.prefijo, r.contador, r.sufijo))
    ...
    def rellenar_widgets(self):
        ...
        self.rellenar_detalles()
        self.objeto.make_swap()
        self.wids['ventana'].set_title(self.objeto.get_info())

    def rellenar_detalles(self):
        """
        Muestra detalles del contador: Última factura creada, siguiente 
        factura a crear y facturas del contador.
        """
        self.wids['e_ultima'].set_text(self.objeto.get_last_numfactura())
        self.wids['e_siguiente'].set_text(self.objeto.get_next_numfactura())
        self.wids['e_numfacturas'].set_text("%d" % 
            len(self.objeto.facturasCompra))
    ...
    def nuevo(self, widget):
        ...
        self.objeto = self.clase(b = False)  
    ...
    def buscar(self, widget):
        ...
        a_buscar = utils.dialogo_entrada(titulo = "BUSCAR %s" % self.clase.sqlmeta.table.upper(), 
                                         texto="Introduzca prefijo o sufijo:", 
                                         padre = self.wids['ventana']) 
        if a_buscar != None:
            try:
                ...
                ida_buscar = -1
            campos_busqueda = (self.clase.q.prefijo, self.clase.q.sufijo)
            subsubcriterios = []
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("series_facturas_compra.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
