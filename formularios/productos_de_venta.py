...
class ProductosDeVenta(Ventana, VentanaGenerica):
    CLASE = pclases.ProductoVenta
    VENTANA = os.path.join("ui", "productos_de_venta.glade")
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana="productos_de_venta.py")
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        filas_res = []
        for r in resultados:
            filas_res.append((r.id, r.nombre, r.familia 
                                              and r.familia.nombre 
                                              or ""))
        id = utils.dialogo_resultado(filas_res,
                                     titulo = 'SELECCIONE %s' % self.clase.sqlmeta.table.upper(),
                                     cabeceras = ('ID', 'Nombre', 'Familia'), 
    ...
    def nuevo(self, widget):
        ...
        self.objeto = self.clase(familia = None, envase = None)  
    ...
    def guardar(self, widget):
        ...
        self.actualizar_ventana()
        self.wids['b_guardar'].set_sensitive(False)
        # DONE: Actualizar precios de albaranes sin facturar si el precio 
        # por defecto ha cambiado. -> Ya no hay precio por defecto en la 
        # el producto (está DEPRECATED), así que no hace falta.
        if (self.objeto.precio 
            and self.objeto.precio not in self.objeto.precios):
            self.objeto.addPrecio(self.objeto.precio)
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("productos_de_venta.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
