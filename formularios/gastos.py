...
class Gastos(Ventana, VentanaGenerica):
    CLASE = pclases.Gasto
    VENTANA = os.path.join("ui", "gastos.glade")
    def __init__(self, objeto = None, usuario = None):
        ...
        connections = {'b_salir/clicked': self.salir,
                       ...
                       'b_buscarFecha/clicked': self.set_fecha, 
                       'b_nuevaFactura/clicked': self.nueva_factura
                      }
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana = "gastos.py")
    ...
    def nuevo(self, widget):
        ...
        try:
            cg = pclases.CuentaGastos.select()[0]
            # Voy a intentar en todo lo posible que no haya gastos sin cuenta.
        except IndexError:
            cg = None
        self.objeto = self.clase(cuentaGastos = cg, 
                                 facturaCompra = None, 
                                 parcela = None)  
    ...
    def buscar(self, widget):
        ...
        a_buscar = utils.dialogo_entrada(titulo = "BUSCAR %s" % self.clase.sqlmeta.table.upper(), 
                                         texto="Introduzca código o concepto:", 
                                         padre = self.wids['ventana']) 
        if a_buscar != None:
            try:
                ...
                ida_buscar = -1
            campos_busqueda = (self.clase.q.codigo, self.clase.q.concepto)
            subsubcriterios = []
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("gastos.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
    ...
    def nueva_factura(self, boton):
        """
        Abre la ventana de facturas con una nueva factura.
        """
        fras_antes = set([fila[0] 
                          for fila 
                          in self.wids['cbe_facturaCompraID'].get_model()])
        import facturas_compra
        vfactura = facturas_compra.FacturasCompra(usuario = self.usuario, 
                                                  nueva = True)
        utils.rellenar_lista(self.wids['cbe_facturaCompraID'], 
                             [(p.id, p.numfactura) for p in 
                          pclases.FacturaCompra.select(orderBy = "numfactura")])
        fras_despues = set([fila[0] 
                            for fila 
                            in self.wids['cbe_facturaCompraID'].get_model()])
        diferencia = fras_despues - fras_antes
        try:
            idnueva_factura = diferencia.pop()
            self.objeto.facturaCompraID = idnueva_factura
        except KeyError:
            pass
        self.actualizar_ventana()
