...
class MateriasActivas(Ventana, VentanaGenerica):
    CLASE = pclases.MateriaActiva
    VENTANA = os.path.join("..", "ui", "materias_activas.glade")
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana = "materias_activas.py")
    ...
    def buscar(self, widget):
        ...
        a_buscar = utils.dialogo_entrada(titulo = "BUSCAR %s" % self.clase.sqlmeta.table.upper(), 
                                         texto = "Introduzca nombre de la materia activa:", 
                                         padre = self.wids['ventana']) 
        if a_buscar != None:
            try:
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("materias_activas.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
