...
###################################################################
## envases.py - Tipos de envase para productos a granel y tarrinas.
###################################################################
## NOTAS:
...
class Envases(Ventana, VentanaGenerica):
    ...
    def inicializar_ventana(self):
        ...
        utils.rellenar_lista(self.wids['proveedorID'], 
                             [(p.id, p.nombre) for p in 
                                pclases.Proveedor.select(orderBy = "nombre")])
        cols = (('Tarrina', 'gobject.TYPE_STRING', False, True, True, None),
                ('Cantidad', 'gobject.TYPE_INT', True, True, True, 
                    self.cambiar_cantidad_empaquetado),
        ...
        self.wids['tv_empaquetado'].connect("row-activated", self.abrir_tarrina)

    def add_empaquetado(self, boton):
        # 1.- Pedir envase.
        tarrinas = [(e.id, "%s (%s gr)" % (e.nombre, utils.float2str(e.gr)))
                   for e in pclases.Tarrina.select(orderBy = "nombre")]
        idtarrina = utils.dialogo_combo("Tarrina", 
            "Seleccione tarrina o flowpack:", 
            tarrinas, 
            padre = self.wids['ventana'])
        # 2.- Crear registro.
        if idtarrina:
            tarrina = pclases.Tarrina.get(idtarrina)
            envase = self.objeto
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana = "envases.py")
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        filas_res = []
        for r in resultados:
            filas_res.append((r.id, r.nombre, r.kg))
    ...
    def abrir_tarrina(self, tv, path, col):
        id = tv.get_model()[path][-1]
        empaquetado = pclases.Empaquetado.get(id)
        tarrina = empaquetado.tarrina
        import tarrinas
        w = tarrinas.Tarrinas(objeto = tarrina, usuario = self.usuario)

    def cambiar_cantidad_empaquetado(self, cell, path, text):
        """
    ...
    def rellenar_tabla_empaquetado(self):
        ...
        for p in self.objeto.empaquetados:
            model.append((p.tarrina and p.tarrina.nombre 
    ...
    def nuevo(self, widget):
        ...
        self.objeto = self.clase(proveedor = None)  
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("envases.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
