...
###################################################################
## facturas_compra.py - Facturas de compra relacionadas con gastos.
###################################################################
## Changelog:
...
class FacturasCompra(Ventana, VentanaGenerica):
    CLASE = pclases.FacturaCompra
    VENTANA = os.path.join("ui", "facturas_compra.glade")
    def __init__(self, objeto = None, usuario = None, nueva = False):
        """
        Constructor. objeto puede ser un objeto de pclases con el que
        comenzar la ventana (en lugar del primero de la tabla, que es
        el que se muestra por defecto).
        Si «nueva» es True, inicia la ventana creando una nueva factura.
        """
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        ws = ["b_borrar"] +\
            ...
            # chequeo de permisos.
        ws.remove("serieFacturasCompraID")
        for w in ws:
            try:
        ...
        if chequear_permisos:
            self.check_permisos(nombre_fichero_ventana = "facturas_compra.py")
    ...
    def nuevo(self, widget):
        ...
        proveedores = [(p.id, "%s (%s)" % (p.nombre, p.cif)) 
                        for p in pclases.Proveedor.select(
                            ...
                            orderBy = "nombre")]
        idproveedor = utils.dialogo_combo("PROVEEDOR NUEVA FACTURA", 
                                          "Seleccione un proveedor:", 
                                          proveedores, 
                                          padre = self.wids['ventana'])
        if idproveedor != None:
            series = [(s.id, s.get_next_numfactura()) for s in 
                        pclases.SerieFacturasCompra.select(orderBy = "prefijo")]
            ...
            if idserie != None:
                ...
                serie = pclases.SerieFacturasCompra.get(idserie)
                numfactura = serie.get_next_numfactura()
                self.objeto = self.clase(proveedorID = idproveedor, 
                                         serieFacturasCompra = serie, 
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("facturas_compra.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
